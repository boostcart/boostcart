...
export default function EditCustomerClient({ user }: { user: User }) {
	...
	const onSubmit = (data: DashboardCustomersNewUserSchemaType) => {
		startTransition(() => {
			editUser(user.id, data).then((callback) => {
				if (callback.error) {
	...
	return (
		<Form {...form}>
			...
			>
				<Header
					title={t("dashboard.customers.editUser.title", {
						userName: user.name,
					})}
					description={t("dashboard.customers.editUser.description")}
				>
					<Button
						variant="secondary"
						type="button"
